%% 创建数据集
function [  ] = creat_dataset_1(  )
    % 创建10000个个体
    [decs,p] = fat(rand(100000,50));
    [c,w] = size(decs);
    filepath = 'F:\多目标优化论文\实验——代码\PlatEMO v1.5 (2017-12)\Data\DMOPs1_OS\DMOPs1_OS.txt';
    if exist(filepath,'file')
        delete(filepath);
    end
    fp = fopen(filepath,'a');
    for p=1:c
        [ET,UD,PET,DSFET,FP,TKS,scheduleplan,DSFR,PIPEFR,FPORDER,RT] = init();
        x = decs(p,1:w);

        for i=1:25      % 25步
            [ TK_NO,DS_NO ] = decode( x,i,ET,UD );
            row = create_data(TKS,PET,TK_NO,DS_NO,ET,UD,DSFR,RT,PIPEFR);
            tmp = '';
            for j=1:length(row)
                tmp = strcat(tmp,sprintf('%.2f',row(j)));
                if j~=length(row)
                    tmp = strcat(tmp,',');
                end
            end
            tmp = strcat(tmp,'\n');
            fprintf(fp,tmp);
            [ Flag,ET,UD,PET,DSFET,FP,TKS,scheduleplan ] = one_step( x,i,ET,UD,PET,DSFET,FP,TKS,scheduleplan,DSFR,PIPEFR,FPORDER,RT );
            if Flag
                break;
            end
        end
...
function [RT,DSFR,PIPEFR,FPORDER,c1,c2,TKS,FP] = parameter_setting()
    ...
    Plan = [38000,41992;21000,49008;30000,120000];      %炼油计划
	c1 = [0 11 12 13 7 15;
            10 0 9 12 13 7;
            13 8 0 7 12 13;
            13 12 7 0 11 12;
	...
	EOP = zeros(1,length(unique(FPORDER)));
	for i=1:size(EO,1)
        k = EO(i,2);
        EOP(k) = EOP(k) + EO(i,3);
	end
    
    Total = zeros(size(EOP));
    FP = zeros(size(FPORDER));
    for i=1:size(FPORDER,1)
        for j=1:size(FPORDER,2)
            OT = FPORDER(i,j);
            Total(OT) = Total(OT) + Plan(i,j);
            FP(i,j) = Plan(i,j) - EOP(OT);          %进料包(可计算原油种类)
        end
    end
end

%% 初始化
function [ET,UD,PET,DSFET,FP,TKS,scheduleplan,DSFR,PIPEFR,FPORDER,RT,c1,c2] = init()
    ...
    ET = [];                          %空闲供油罐集合
    UD = [1,2,3];                  %未实现炼油计划的蒸馏塔集合
    scheduleplan = [];           %调度信息记录表
    ...
    for i=1:size(TKS,1)  
        if TKS(i,2)~=inf                                        %过滤出空罐
            ...
            % 更新供油罐的状态
            TKS(i, 4) = ds;   %记录蒸馏塔
            TKS(i, 5) = Temp;   %记录供油开始时间
            TKS(i, 6) = DSFET(ds);   %记录供油结束时间
        end
    end
end

%% 计算解的适应度
function [ eff,scheduleplans ] = fat( pop )
    %变量取值范围[0,1]
    %决策变量维数w
    %目标函数个数M=4
    ...
    for p = 1:popsize    
        ...
        x = pop(p,1:w);    
        % 初始化
        [ET,UD,PET,DSFET,FP,TKS,scheduleplan,DSFR,PIPEFR,FPORDER,RT,c1,c2] = init();
        %首先判断当前状态是否可以调度
...
%% 创建数据
function [row] = create_data(TKS,PET,TK_NO,DS_NO,ET,UD,DSFR,RT,PIPEFR)
    %更新库存的信息
    for i = 1:size(TKS, 1)          %j代表供油罐
        if TKS(i,2) ~= inf
            if TKS(i, 5) < PET && TKS(i, 6) > PET
                TKS(i, 1) = TKS(i, 3) - (PET - TKS(i, 5)) * DSFR(TKS(i, 4));
            end
        end
    end
    if TK_NO == 0
    ...
    row = [TKS(:,1)',DSFR,PIPEFR,RT,TKS(:,3)', total,TK,DS];
end

%% 解码
function [ TK_NO,DS_NO ] = decode( X,step,ET,UD )
    %供油罐
    TK_NO=getInt(X(2*step-1), size(ET, 2));
    %蒸馏塔
    DS_NO = getInt(X(2*step), size(UD, 2)-1)+1;
end

%% 回溯调度(只需要找到一个可行解即可)
function [ Flag,ET,UD,PET,DSFET,FP,TKS,SchduleTable ] = one_step( X,step,ET,UD,PET,DSFET,FP,TKS,SchduleTable,DSFR,PIPEFR,FPORDER,RT )
% 只需要依次遍历下去，若出现不可调度的地方就回溯
% 然后接着遍历，直到FP为空
    Flag = false;%是否找到可行解的标志

    %结束条件
    if round(sum(sum(FP))) == 0 || isempty(UD)
        Flag = true;
    else
        [ TK_NO,DS_NO ] = decode( X,step,ET,UD );
        ...
        if TK_NO==0 
            ...
            DS = UD(DS_NO);
            %转运
            [~,PET,TKS,FP,ET,UD,DSFET,SchduleTable] = tryschedule(TK,DS,DSFET,PET,PIPEFR,RT,ET,UD,DSFR,TKS,FP,FPORDER,SchduleTable);
        end
    end
end
...
function [SchduleTable,TKS,ET,PET] = stop(ET,PET,TKS,SchduleTable,PipeStoptime,DSFR)
	...
	%更新转运记录表，不需要挑选供油罐供油
	SchduleTable = [SchduleTable; length(DSFR) + 1, 0, PETOLD, PET, 0, 0, 0];
end
...
function [Flag,PET,TKS,FP,ET,UD,DSFET,SchduleTable] = tryschedule(TK,DS,DSFET,PET,PIPEFR,RT,ET,UD,DSFR,TKS,FP,FPORDER,SchduleTable)
	...
	if V >= 150
    	...
    	%更新转运记录和炼油记录
        SchduleTable = [SchduleTable;  length(DSFR)+1, TK, PETOLD, PET, COT, 0, V];
    	SchduleTable = [SchduleTable;  DS, TK, DSFETOLD, DSFET(DS), COT, 0, V];
        %判断DS是否炼油成功
