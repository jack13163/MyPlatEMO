%% 计算解的适应度
function [ eff,scheduleplans ] = fat_3( pop )
    ...
    for p = 1:popsize
        % 参数设置
        [RT,DSFR,PIPEFR,FPORDER,c1,c2,c3,TKS,FP] = parameter_setting();
        ...
        if schedulable(FPORDER, DSFR, PIPEFR, RT, TKS, PET, FP, UD) 
            ...
            for i=1:size(TKS,1)
                ...
                if TKS(i,2)~=inf
                    ...
                    scheduleplan = [scheduleplan; ds, i, Temp, DSFET(ds), TKS(i,2),0,TKS(i,3)];
                    TKS(i, 4) = ds;   %记录蒸馏塔
        ...
        new_scheduleplan=sortrows(scheduleplan,1);

        if Flag
            %计算适应度函数
            f1 = gdmix(new_scheduleplan, c1);      %管道混合成本
            f2 = gdimix(new_scheduleplan, c2);     %罐底混合成本
            f3 = gchange(new_scheduleplan);        %蒸馏塔的供油罐切换次数
            f4 = gnum(new_scheduleplan);              %供油罐个数
            f5 = roundn(energecost(new_scheduleplan, c3),0);     %能耗成本
        else
            f1 = inf;
            f2 = inf;
            f3 = inf;
...
%% 参数设置
function [RT,DSFR,PIPEFR,FPORDER,c1,c2,c3,TKS,FP] = parameter_setting()
    %% 参数设置(不变参数)    
   	RT = 6;                                                     %驻留时间
	DSFR = [333.3, 291.7, 625];                  %蒸馏塔炼油速率(可计算蒸馏塔个数)
    PIPEFR = [833.3 1250 1375];               %几组不同的管道输油速率
    ...
    elseif step <= 25   %步长不得超过25
        ...
        while ~all(footprint) && ~Flag
            ...
            DS_NO = getInt(X(3*step-1), size(UD, 2)-1)+1;
          	DS = UD(DS_NO);
          	%转运速率
           	R = getInt(X(3*step), 3-1)+1;       %三种速率
                       
           	%可行性判断
           	f1 = false;
           	if TK_NO==0
              	PipeStoptime  = roundn(getPipeStoptime(DSFR, RT, TKS, PET, UD),-6);%四舍五入，取6位小数
              	if PipeStoptime > 0 
                  	f1 = true;
                 	%停运
              	...
              	%试调度
              	[f2,PET,TKS,FP,ET,UD,DSFET,SchduleTable] = tryschedule(ET(TK_NO),DS,DSFET,PET,PIPEFR,RT,ET,UD,DSFR,TKS,FP,FPORDER,SchduleTable,R);
              	...
              	for i = 1:length(footprint)
                    if footprint(i) == 0
                     	%footprint中，第1位代表管道停运
                       	if i == 1
                          	while 0 ~= getInt(X(3*step-2),size(ET, 2))
                             	X(3*step-2) = rand;
                            end
                         	footprint(1) = 1;%停运标记
                      	elseif isempty(ET)    %已经尝试过停运不行，并且无空罐
                          	...
                          	while (i-1) ~= getInt(X(3*step-1),size(UD, 2)-1)+1
                                X(3*step-1) = rand;
                            end
                         	footprint(i) = 1;%蒸馏塔标记
                        end
...
%% 试调度
function [Flag,PET,TKS,FP,ET,UD,DSFET,SchduleTable] = tryschedule(TK,DS,DSFET,PET,PIPEFR,RT,ET,UD,DSFR,TKS,FP,FPORDER,SchduleTable,R)
	%原油类型
	for i=1:size(FPORDER,2)
        if FP(FPORDER(DS, i)) ~= 0
            COT = FPORDER(DS, i);      %原油类型
            break;
        end
	end
	%库存总油量
    ...
    %安全的原油体积
    V = roundn(getVolume(DS,DSFET,PET,RT,UD,total,DSFR,PIPEFR(R)),-6);
	%体积不可以低于特定的阈值
	if V >= 150
        if TKS(TK, 1) < V
            V = TKS(TK, 1);
        ...
        %转运期间释放的供油罐添加到供油罐集合
        for i = 1:size(TKS,1)
            if (TKS(i,6) > PET && TKS(i,6) <= PET + V / PIPEFR(R))
                %将当前油罐添加到ET中
                ET = [ET, i];
                %供油罐信息清空
                TKS(i, 2) = inf;
    	...
    	%更新转运记录和炼油记录
        SchduleTable = [SchduleTable;  length(DSFR)+1, TK, PETOLD, PET, COT,R,V];     %只记录使用的速率序号
    	SchduleTable = [SchduleTable;  DS, TK, DSFETOLD, DSFET(DS), COT,R,V];
        %判断DS是否炼油成功
...
%% 获取管道停运安全时间
function [ t ] = getPipeStoptime(DSFR, RT, TKS, PET, UD)
    t=inf;
    ...
    for i=1:K
        ...
        if tag(i) == 0
            ...
            if beta > 0 && beta < K * arfai && available(i,2) >= K * arfai - beta
                time = [time, cur + beta / max(PIPEFR)];
            end
            cur = cur + K * arfai / max(PIPEFR);
        end
    end
