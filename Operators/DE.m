...
% <operator> <real>
% Differental evolution and polynomial mutation(差分进化与多项式突变)
% CR   ---   1 --- Parameter CR in differental evolution
% F    --- 0.5 --- Parameter F in differental evolution
% proM ---   1 --- The expectation of number of bits doing mutation(突变比特数的期望)
% disM ---  20 --- The distribution index of polynomial mutation(多项式变异的分布指数)
...
%--------------------------------------------------------------------------
    ...
    Site = rand(N/3,D) < CR;
    OffspringDec(Site) = OffspringDec(Site) + F*(Parent2Dec(Site)-Parent3Dec(Site));    %传统的变异交叉操作

    %% Polynomial mutation（模拟二进制编码情况下的单点变异）
