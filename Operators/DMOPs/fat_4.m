%% 计算解的适应度
function [ eff,scheduleplans ] = fat_4( pop )
...
function [RT,DSFR,PIPEFR,FPORDER,c1,c2,c3,TKS,FP] = parameter_setting()
 	%% 参数设置(不变参数)
   	RT = 6;                                                     %驻留时间
	DSFR = [375,230,500];                  %蒸馏塔炼油速率(可计算蒸馏塔个数)
    PIPEFR = [833.3 1250 1375];               %管道的输油速率
    FPORDER = [3,1,1;2,2,2;4,5,6];                     %蒸馏塔的炼油顺序
	c1 = [0 11 12 13 7 15;
            10 0 9 12 13 7;
            13 8 0 7 12 13;
            13 12 7 0 11 12;
	...
	EOP = zeros(1,max(max(FPORDER)));
	for i=1:size(EO,1)
        k = EO(i,2);
        EOP(k) = EOP(k) + EO(i,3);
	end
	Total = [63000 55200 27000 27000 55000 38000];
	FP = Total - EOP;               %进料包(可计算原油种类)
end
